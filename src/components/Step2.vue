...
import { troubleShooting } from "@/utils/fileSource";
...
const setStep = (key) => {
  if (key == "") {
    ...
    if (key == 4) {
      ...
      let num = 1;
      data.getSource = troubleShooting(num, stateData.role);
    }
    nextStep(key);
  }
...
const playOpen = (item) => {
  ...
  //   data.getSource = troubleShooting(item.key, stateData.role);
  //   map.showMap = false;
  //   data.stepVal = 1;
  //   videoPlay();
  // })
